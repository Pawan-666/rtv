...
Use a Python virtual environment and install from the local source tree.

Prerequisites: Python 3.8+, git, a Unicode-capable terminal.
...
# upgrade packaging tools and install RTV in editable mode
python -m pip install -U pip setuptools wheel
pip install -e .

# install mailcap support (needed on newer Python versions)
...
Alternative: run directly as a module after installing as above
...
Tips:
- If you see garbled characters, try `rtv --ascii`.
- To create a user config: `rtv --copy-config`.
